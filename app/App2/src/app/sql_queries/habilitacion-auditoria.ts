...
USE master;
CREATE SERVER AUDIT Auditoria_BD1 TO FILE ( FILEPATH = 'E:\\Auditoria3' )`;

export const habilitacion_auditoria = `
USE master; 
ALTER SERVER AUDIT Auditoria_BD1  WITH (STATE = ON)`;

export const espec_audit_bd = `
USE Orders;
CREATE DATABASE AUDIT SPECIFICATION Audit_Data_Modification FOR SERVER AUDIT Auditoria_BD1 
ADD ( SELECT, INSERT, UPDATE, DELETE ON Schema::dbo BY public), ADD ( SCHEMA_OBJECT_CHANGE_GROUP) 
WITH (STATE = ON)`;

export const prefijo_archivo_log = `SELECT log_file_name from sys.server_file_audits where name='Auditoria_BD1';`;
...
export const log_auditoria = (prefijo: string) => {
  return `SELECT distinct fn.event_time, 
	   ...
	   fn.object_name, 
	   fn.statement    
 FROM sys.fn_get_audit_file ('E:\\Auditoria3\\${prefijo}', DEFAULT, DEFAULT) as fn
